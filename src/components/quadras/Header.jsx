...
const links = [["/home", "Início", "🏠"], ["/quadras", "Quadras", "🏐"], ["/agendar-horario", "Agendar horário", "🕐"], ["/agendamentos", "Agendamentos", "📅"], ["/torneios", "Torneios", "🏆"], ["/pagamento", "Pagamento", "💳"]];
...
export default function Header() {
  ...
  return <>
    <header className={styles.topHeader}>
      <button type="button" className={styles.menuButton} onClick={() => setMenuAberto(true)} aria-label="Abrir menu"><FaBars /></button>
    ...
    <aside className={`${styles.sidebarMenu} ${menuAberto ? styles.open : ""}`} aria-hidden={!menuAberto}>
      <div className={styles.sidebarBrand}><img src={logo} alt="Arena Beach" className={styles.sidebarLogo} /><div className={styles.brandText}><strong>ARENA</strong><strong>BEACH</strong></div><button type="button" className={styles.closeMenu} onClick={close} aria-label="Fechar menu"><FaTimes /></button></div>
